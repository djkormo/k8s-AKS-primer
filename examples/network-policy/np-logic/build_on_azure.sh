...
then
echo "Building image...";
az acr build --registry $ACR_NAME --image np-logic:v1 .
# szczegoly 
az acr repository show -n $ACR_NAME -t np-logic:v1
fi

if [ "$OPERATION" = "rebuild" ] ;
then
echo "Rebuilding image...";
az acr repository delete --name $ACR_NAME --repository np-logic:v1
az acr build --registry $ACR_NAME --image np-logic:v1 .
# szczegoly 
az acr repository show -n $ACR_NAME -t np-logic:v1
fi
